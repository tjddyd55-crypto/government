...
export function registerGovernmentSignatureTemplateApi(apiRouter, ctx) {
  ...
  apiRouter.post('/government-support/signatures/send', ...chain, async (req, res) => {
    ...
    try {
      ...
      const cust = await assertProfileForGovSignatureSend(client, profileId, getAuthUserId(req))
      if (cust.error) {
      ...
      let governmentSignatureTemplatesOrdered = /** @type {{ id: string, title: string, version: number, required: number, pdfHash: string | null, pdfTemplateId: number | null }[]} */ ([])

      await client.query('BEGIN')

      if (hasPkg) {
        const pid = String(pPkg).trim()
        if (!pid.startsWith(GSPKG_PREFIX)) {
          await client.query('ROLLBACK')
          res.status(400).json({ ok: false, message: 'packageId 형식이 올바르지 않습니다.' })
          return
        }
        const pacc = await assertPackageAccess(client, pid, ownerUserId, isSuper)
        if (pacc.error) {
          await client.query('ROLLBACK')
        ...
        governmentSignatureTemplatesOrdered = items.rows.map((x) => ({
          id: x.id,
          title: x.title,
          version: x.version,
      ...
      if (hasTpl) {
        ...
        governmentSignatureTemplatesOrdered = []
      ...
      const senderMaps = senderValuesByGovernmentSignatureTemplates(
        senderRoot,
        governmentSignatureTemplatesOrdered.map((x) => String(x.id)),
      )
      ...
      for (const ct of governmentSignatureTemplatesOrdered) {
    ...
    try {
      ...
      if (!isSuper) {
        ...
        if (normalizeRbacRole(req.user?.role) === 'USER') {
          ...
          where += ` AND p.owner_user_id = $${params.length}`
        }
      }
      const r = await pool.query(
        `
        SELECT s.id, s.sign_token, s.profile_id, s.status, s.target_phone_masked, s.sent_at, s.created_at, s.package_id
        ${where}
